	...
	public int uploadHdfExcel(HttpServletRequest request, HttpServletResponse response, MultipartFile file)
		...
		try {
			...
			for (int i = 1; i < listob.size(); i++) {
				...
				sapHdfTb.setSkly(ObjectUtils.getString(listob.get(i).get(7)));
				sapHdfTb.setSapjxsbm(ObjectUtils.getString(listob.get(i).get(8)));
				sapHdfTb.setSrkm(ObjectUtils.getString(listob.get(i).get(9)).split("/")[1]);
				sapHdfTb.setXssje(ObjectUtils.getString(listob.get(i).get(10)));
				list.add(sapHdfTb);
			}
...
//		    String str=df.format(Double.parseDouble(sapHdfTb.getHdf().trim())/1.06);
			...
			token20.setValue(sapHdfTb.getSrkm());
			...
			amountItem.setAmountContent(new BigDecimal(-(Double.parseDouble(sapHdfTb.getHdf())-Double.parseDouble(sapHdfTb.getXssje())))); 
			...
			amountItem1.setAmountContent(new BigDecimal(-Double.parseDouble(sapHdfTb.getXssje()))); 
	...
	public XSSFWorkbook getXSSFWorkbook(String excelUrl,String hdfStatus, String zkrq) {
		...
		try {
			...
			for (SapHdfTb SapHdfTb : item) {
				...
				cell7.setCellValue(SapHdfTb.getSkly());
				
				XSSFCell cell8 = row.getCell(8);
				if (null == cell8) {
					cell8 = row.createCell(8);
				}
				cell8.setCellValue(SapHdfTb.getSapjxsbm());
				
				XSSFCell cell9 = row.getCell(9);
				if (null == cell9) {
					cell9 = row.createCell(9);
				}
				cell9.setCellValue(SapHdfTb.getSrkm());
				
				XSSFCell cell10 = row.getCell(10);
				if (null == cell10) {
					cell10 = row.createCell(10);
				}
				cell10.setCellValue(SapHdfTb.getXssje());
