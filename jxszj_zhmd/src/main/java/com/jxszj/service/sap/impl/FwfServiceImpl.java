	...
	public int uploadFwfExcel(HttpServletRequest request, HttpServletResponse response, MultipartFile file)
		...
		try {
			...
			for (int i = 1; i < listob.size(); i++) {
				...
				SapFwfTb.setSkly(ObjectUtils.getString(listob.get(i).get(7)));
				SapFwfTb.setSapjxs(ObjectUtils.getString(listob.get(i).get(8)));
				SapFwfTb.setFwfkm(ObjectUtils.getString(listob.get(i).get(9)).split("/")[1]);
				SapFwfTb.setXxs(ObjectUtils.getString(listob.get(i).get(10)));
				list.add(SapFwfTb);
			}
...
//		    String str=df.format(Double.parseDouble(SapFwfTb.getFwf().trim())/1.06);
			...
			token20.setValue(SapFwfTb.getFwfkm());
			...
			amountItem.setAmountContent(new BigDecimal(Double.parseDouble(SapFwfTb.getFwf()))); 
			...
			amountItem1.setAmountContent(new BigDecimal(Double.parseDouble(SapFwfTb.getXxs()))); 
	...
	public XSSFWorkbook getXSSFWorkbook(String excelUrl,String fwfStatus, String zkrq) {
		...
		try {
			...
			for (SapFwfTb SapFwfTb : item) {
				...
				cell7.setCellValue(SapFwfTb.getSkly());
				
				XSSFCell cell8 = row.getCell(8);
				if (null == cell8) {
					cell8 = row.createCell(8);
				}
				cell8.setCellValue(SapFwfTb.getSapjxs());
				
				XSSFCell cell9 = row.getCell(9);
				if (null == cell9) {
					cell9 = row.createCell(9);
				}
				cell9.setCellValue(SapFwfTb.getFwfkm());
				
				XSSFCell cell10 = row.getCell(10);
				if (null == cell10) {
					cell10 = row.createCell(10);
				}
				cell10.setCellValue(SapFwfTb.getXxs());
