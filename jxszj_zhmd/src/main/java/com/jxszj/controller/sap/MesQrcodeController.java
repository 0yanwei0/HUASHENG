...
//	}
	...
	@ResponseBody
	public MessageResult addMesQrcodeTb(String sjxdrq){
		if("".equals(sjxdrq)){
			sjxdrq=DateUtils.getNowDateToString();
		}
		Map<String,Long> jhzqs=new HashMap<String,Long>();
...
//        int ddTotal=0;
		...
		try {
	        ...
	        //查询出交货周期表所有的销售订单，并去重
	        JSONArray array=getJSONArray("https://my300201-api.saps4hanacloud.cn/sap/opu/odata/sap/YY1_MANUFACORDERAPI_CDS/YY1_ManufacOrderAPI?$filter=MfgOrderActualReleaseDate%20eq%20datetime'"+sjxdrq+"T00:00:00'");
	        Set<String> sets=new HashSet<String>();//存放销售订单
	        for (int i = 0; i < array.size(); i++) {
